...
We present our streamlining procedure as an operator, called the \emph{normaliser}. The normaliser can be thought of as a `shape' with holes. To produce a weakly streamlined derivation, a derivation is inserted into the holes and the atoms of the derivation are associated with parts of the shape. Features of the shape are analogous to properties of a normalisation procedure. The fact that the shape is finite gives termination, its size givse the complexity, its asymmetries gives non-determinism, etc.
...
Note that an atomic flow associated to a proof has no upper edges, so the top left and the two bottom left boxes in the above atomic flow would be empty.
...
We will now show how weakly streamline a derivation. This is done in two steps. First we extract the \emph{core} of the derivation, then we construct the \emph{normaliser}. The normaliser is used to plug several copies of the core together to create a weakly streamlined derivation.

\subsection{The Core}

In this subsection we wish to find a way to modify a derivation to make it weakly streamlined. Since weak streamlining is defined in terms of atomic flows, we will define what we do to atomic flows first and then show that an analogous transformation  can be done to derivations later. The \emph{core of an atomic flow} is obtained by removing each connected pair of axiom and cut vertices. As can be seen in Definition~\ref{DefFlowCore} below this leaves most of the atomic flow untouched and it produces a weakly streamlined atomic flow. The definition of the \emph{core of a derivation} follows naturally.
...
\]
where, for $1\leq i\leq n$, $\phi_i$ is connected and contains no (co)interaction vertices, the \emph{core of $A$} is defined to be
\[
\Core(A)=
\atomicflow
{
...
Given a derivation $\vlder{\Phi}{}{\beta}{\alpha}$ with associated atomic flow $A$, a \emph{core of\/ $\Phi$} is defined to be a derivation $\vlder{}{}{\vls[\beta.(a_n.{\bar a_n}).\cdots.(a_1.{\bar a_1})]}{\vls([a_1.{\bar a_1}].\cdots.[a_n.{\bar a_n}].\alpha)}$ with associated atomic flow $\Core(A)$.
\end{definition}
...
A core of a derivation is weakly streamlined.
\end{proposition}
...
\begin{definition}
Given a derivation $\Phi$, a core of $\Phi$ obtained as described in the proof of Theorem~\ref{ThmExistCore} is called \emph{the core of $\Phi$}, denoted $\Core(\Phi)$.
\end{definition}
...
\begin{itemize}
 ...
 \item $\Norm(\Phi,a_1,\dots,a_k)$ is weakly streamlined.
\end{itemize}
\end{lemma}
