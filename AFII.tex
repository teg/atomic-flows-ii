...
\begin{proof}
First note that any atom that did not occur in both interaction and cointeraction instances in $\Phi$, will not do it in $\Norm(\Core(\Phi),a_1,\dots,a_n)$ either.
Consider the atomic flow of any atom $a_i$ from $a_1,\dots,a_n$. Note that the only interaction and cointeraction instances it appears in are the ones shown in $\Norm(\Core(\Phi),a_1,\dots,a_i)$. Now by studying the atomic flow of $\Norm(\Core(\Phi),a_1,\dots,a_i)$,
\begin{center}
\includegraphics[width=150mm]{./threeboxes.ps}
\end{center}
it is easy to observe that there could not be any path from the interaction vertex to the cointeraction vertex.
