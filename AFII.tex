...
The idea behind the normalisation procedures we are about to present is to copy the derivation we wish to normalise, modify each copy and combine all the copies to produce a derivation on normal form. We will now establish some preliminary results needed later on.

First we observe that we are allowed to make arbitrary copies of any formula.
...
We now present a strongly normalising and confluent cut-elimination procedure for proofs. It works in the same spirit as above, we make copies of the original derivation, alter them and plug them together. However, in the case of streamlining we alter all the derivations in the same way, \emph{i.e.} we extract the core, and the context in which we plug the derivations differed. Now we will alter each proof in a different way, but the context in which we plug them will all be the same. The reason we claim this procedure to be confluent is that given the atomic flow of a proof, the atomic flow of the resulting cut free proof is unique (modulo associativity of contraction).
...
From the point of view of atomic flows this proof is symmetric, that is the atomic flows of each of the atoms are pairwise isomorphic. When building the proof we have to make a choice about which atom to eliminate first, but the atomic flows forget this choice:
...
Finally we build a cut-free proof, which is basically saying `consider every truth value assignment to the atoms in $\Phi$ and prove $\beta$ under each of them'.
...
Finally we observe that the atomic flow of the cut-free proof can be uniquely determined given the atomic flow of the original proof:
