...
\begin{defi}\label{DefBotExperiment}
Given a derivation $\Phi$ from $\alpha$ to $\beta$ and an atom $a$ which occurs in $\Phi$, a \emph{$\bot$-experiment on $\Phi$ with respect to $a$} is a derivation $\Phi'$ obtained from $\Phi$ as follows:
\begin{itemize}
 \item given a formula $\xi$ in $\Phi$, let $i$ be the number of cointeractions applied to $a$ above $\xi$, then the formula $\xi'$ is obtained by substituting $\ttt$ for $a$ in $\xi$ and the formula $\delta_i$ is defined by: $\delta_0=\fff$ and $\delta_{i+1}=\vls[\delta_i.{\bar a}]$; \item each rule instance $\vlinf{\rho}{}{\zeta}{\xi}$, where $\rho\in\{\acd,\acu,\awu\}$, which is applied to $a$, is substituted with $\vlinf{=}{}{\vls[\zeta'.\delta_i]}{\vls[\xi'.\delta_i]}$;
 \item each rule instance $\vlinf{\awd}{}{\xi\{a\}}{\xi\{\fff\}}$ is substituted with $\vlinf{\awu}{}{\vls[\xi'\{\ttt\}.\delta_i]}{\vls[\xi'\{\fff\}.\delta_i]}$;
 \item each rule instance $\vlinf{\aid}{}{\xi\{\vls[a.{\bar a}]\}}{\xi\{\ttt\}}$ is substituted with $\vlinf{\awd}{}{\vls[\xi'\{\vls[\ttt.{\bar a}]\}.\delta_i]}{\vls[\xi'\{\ttt\}.\delta_i]}$;
 \item each rule instance $\vlinf{\aiu}{}{\xi\{\fff\}}{\xi\{\vls(a.{\bar a})\}}$ is substituted with $\vlinf{\sus}{}{\vls[\xi'\{\fff\}.\delta_{i+1}]}{\vls[\xi'\{\vls(\ttt.{\bar a})\}.\delta_i]}$; and
 \item each other rule instance $\vlinf{\rho}{}{\zeta}{\xi}$ is substituted with $\vlinf{\rho}{}{\vls[\zeta'.\delta_i]}{\vls[\xi'.\delta_i]}$.
\end{itemize}
\end{defi}

We now observe that an $\top$-experiment indeed gives us the what we want. The observant reader might note that instead of the original conclusion, $\beta$, we get $\beta'$, which in this case is $\beta$ with some atom occurences substituted with $\fff$, as we shall se later, this is not a problem. We could indeed fix this straight away by applying $\awd$'s.

\TODO{Define what it means to do an experiment with respect to a \emph{selection of atoms} instead of just a single atom. A \emph{selection} is a set with some restrictions.}

\begin{defi}
Let $\Phi$ be a derivation and $A$ a set of atoms, then $A$ is called a \emph{selection of atoms from $\Phi$} if $A$ is the smallest set such that for every $a$ which occurs in a (co)interaction in $\Phi$ exactly one of $a$ and $\bar a$ are in $A$.
\end{defi}
...
%\begin{rem}
%If we define a $\bot$-experiment to be the dual of a $\top$-experiment in the usual way, then an \emph{experiment} on $\Psi$ is exactly a $\bot$-experiment on a $\top$-experiment on $\Psi$ (or the other way around) (under the same polarity assignment, with respect to the same atomic flow).
%\end{rem}
...
\TODO{Rewrite with respect to the new definition of experiment.}
...
\TODO{Define what the contraction with dots in the middle means in the following atomic flow.}

\begin{thm}
Reduction $\rightarrow_{is}$ is sound.
\end{thm}
\TODO{Change the definition so that ``the atom mapping to $B$'' does not map to anything else...}
\begin{proof}
Let $\Phi$ be a derivation with flow $C$, such that $C\rightarrow_{si} D$. We show that there exists a derivation $\Psi$ with flow $D$ and with the same premiss and conclusion as $\Phi$. In the following we refer to the figure in Definition~\ref{DefRedIS}. We assume that $\Phi$ has premiss $\xi$ and conclusion $\zeta$. Let $a$ be the atom mapping to the subflow $B$. Now let $\Phi^\top$ be the $\top$-experiment and let $\Phi^\bot$ be the $\bot$-experiment on $\Phi$, both with respect to $a$. Then we have:
\[
\vlder{\Phi^\bot}{}{\vls[\zeta.{\bar a}.\cdots.{\bar a}]}{\xi}
\quad,\quad
\vlder{\Phi^\top}{}{\zeta}{\vls({\bar a}.\cdots.{\bar a}.\xi)}\quad.
\]
Notice that since $B$ is isolated the formulae $\xi$ and $\zeta$ are the same in the experiments as in the original derivation. We can now connect the two experiments to get the desired derivation:
\[
\vlderivation{
\vlin{\cod}{}{\zeta}{
\vlde{\vls[\zeta.\Phi^\top]}{}{\vls[\zeta.\zeta]}{
\vlin{\acu^\star}{}{\vls[\zeta.({\bar a}.\cdots.{\bar a}.\xi)]}{
\vlin{\swi}{}{\vls[\zeta.({\bar a}.\xi)]}{
\vlin{\acd^\star}{}{\vls([\zeta.{\bar a}].\xi)}{
\vlde{\vls(\Phi^\bot.\xi)}{}{\vls([\zeta.{\bar a}.\cdots.{\bar a}].\xi)}{
\vlin{\cou}{}{\vls(\xi.\xi)}{
\vlhy{\xi}}}}}}}}
}\quad.
\]
