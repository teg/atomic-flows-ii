...
%---------------------------------------
\begin{rem}
If an $\ai$-path is maximal, then it is cycle-free.
\end{rem}
...
% TODO: don't use Core \Phi ^\star, call it \Psi

\newcommand{\mk}[1]{{#1}^{\scriptscriptstyle\bullet}}

\begin{proof}
Mark the cones of the atom occurrences in the conclusion of $\Core(\Phi)$ which are not in $\beta$ and which are in $a_1,\dots,a_n$ as well as the cones of both the atom occurrences occurring in cointeraction instances in $\Core(\Phi)$ with $\bullet$. Then perform the following substitutions to obtain $\Core(\Phi)^\star$:
\begin{itemize}
  \item substitute each occurrence of $\mk a$ with $\fff$ and
  \item substitute each structural inference rule in which $\mk a$ occurs as follows
\[
\begin{array}{c}
\vlinf{\awd}{}{\mk a}{\fff}
\quad\rightarrow\quad
...
\]
Notice that the conclusion consists of the same number of occurrences of each atom and every atom occurs in exactly one interaction instance. It follows that each atom occurs in the same number of cocontraction instances and hence the atomic flow of each atom is the same and the proof is canonical.
\end{proof}
...
\begin{proof}
Let $\{a_1,\dots,a_n,\bar a_1,\dots,\bar a_n\}$ be the set of atoms in $\Phi$ which occur in both interaction and cointeraction instances and let $A$ be an enumeration of all the possible choices of $n$ distinct, non-dual atoms from this set. Now build the canonical derivation:
\[
\vlderivation
{
 \vlde{}{\{\acd,\med\}}{\alpha}
