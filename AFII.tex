...
Given all the possible truth value assignments to a set of atoms, \emph{i.e.} the premisses of all the experiments, we know that one of them is true, so there must be a proof of their disjunction.
...
Furthermore, since we want to avoid making any arbitrary choices, the atomic flow of each atom in this proof must be the same, so the order in which the atoms interact does not make a difference:
