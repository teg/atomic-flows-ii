...
We then obtain $\Psi$ from $\Phi$ by substituting each formula $\xi$ with $\vls[(\xi'.\gamma_i^\top).\gamma_j^\bot]$; where $i$ (resp. $j$) is the number of evidenced interactions (resp. cointeractions) below (resp. above) $\xi$ in $\Phi$ such that the edge connecting the corresponding node to $A$ is positive (resp. negative) under $\pi$, $\gamma_0^\top=\ttt$, $\gamma_{i}^\top=\vls(\gamma_{i-1}^\top.a_i)$, $\gamma_0^\bot=\fff$ and $\gamma_{j}^\bot=\vls[\gamma_{j-1}^\bot.a_j]$; where $a_i$ (resp. $a_j$) is the atom, mapping to a positive edge, taking part in interaction (resp. cointeraction) number $i$ (resp. $j$) from the top (resp. bottom) in $\Phi$.
...
For any proof\/ $\vlproof{\Pi}{\SKS}{\beta}$ and any polarity assignment to its atomic  flow, $\pi$, the $\fff$-experiment on $\Pi$ under $\pi$ is of the form $\vlder{}{\SKS\setminus\{\aid,\aiu\}}{\beta'}{\gamma^\top}$. Where $\beta'$ and $\gamma^\top$ are given by Definition~\ref{DefExperimentDerivation}.
\end{pro}

\begin{lem}\label{LemGlueTop}
Let $\Pi$ be a proof, let $\mathcal{P}$ be the set of all the polarity assignments to the atomic flow of $\Pi$ and for each $\pi\in\mathcal{P}$ let the $\fff$-experiment on $\Pi$ under $\pi$ have premiss $\gamma_\pi$. Then there exists a proof $\vlproof{}{\{\aid,\acu,\swi\}}{\bigvee_{\pi\in\mathcal{P}}\gamma_\pi}$.
\end{lem}
