...
\begin{rem}
For simplicity we fix an partial order of atoms. The constraints of the ordering are:
\begin{itemize}
\item $a<b$ or $b<a$ if $a$ and $b$ are distinct, non-dual atoms and
\item $a<b$ implies $\bar a<b$.
\end{itemize}
This allows us to refer to a set of atoms as an enumeration of atoms and denote it $a_1,\dots,a_n$ if the atoms are pairwise non-dual or $a_1,\bar a_1,\dots,a_n,\bar a_n$ if they are closed under duality. When we refer to `the first $k$ atoms from a set of atoms', it is with respect to this ordering.

The ordering is arbitrary and in many cases different choices of orderings give the same results. We will point this out as we go along. In those cases the only reason for the need of an ordering is that the representation we are using (formulae and derivations) are linear and force a choice.
\end{rem}
...
\begin{defi}
Given a derivation $\Phi$ from $\alpha$ to $\beta$, where $a_1,\bar a_1,\dots,a_n,\bar a_n$ are all the distinct atoms that appear in both interaction and cointeraction instances, a \emph{core of\/ $\Phi$} is defined as a derivation 
\[
...
\begin{pro}
Given two canonical cores of a derivation, possibly under two different orderings of atoms, the atomic flows of the two cores are equal.
\end{pro}
...
\begin{itemize}
 ...
 \item an enumeration of pairwise non-dual atoms $a_1,\dots,a_n$,
\end{itemize}
...
\begin{defi}
Given a set of $2n$ atoms, $\mathcal A$, which is closed under duality, a \emph{choice of $\mathcal A$} is a subset of $\mathcal A$ of size $n$ where no two atoms are dual.
\end{defi}

\begin{rem}
Given a choice of a set of $2n$ atoms closed under duality, for every dual pair, $a,\bar a$ in the original set the choice contains exactly one atom $a$ or $\bar a$. The possible number of choices is therefore $2^n$.
\end{rem}

\begin{lem}\label{LemConstrExp}
Given a derivation $\Phi$ from $\alpha$ to $\beta$, let $\mathcal A$ be all the atoms occurring in both interaction and cointeraction instances, then given a choice of $\mathcal A$, $a_1,\dots,a_n$, there exists an experiment on $\Phi$ with respect to $a_1,\dots,a_n$.
\end{lem}

\newcommand{\mk}[1]{{#1}^{\scriptscriptstyle\bullet}}

\begin{proof}
Consider $\Core(\Phi)$ from $\vls([a_1.\bar a_1].\cdots.[a_n.\bar a_n])$ to $\vls[\beta.(a_1.\bar a_1).\cdots.(a_n.\bar a_n)]$. Note that this derivation still contains cointeraction instances, but since $\Phi$ is a proof and by the definition of $\Core$, every path in the atomic flow of $\Core$ from a cointeraction nodes goes to a weakening node.
...
\]
for some $b_1,\bar b_1,\dots,b_m,\bar b_m,\xi_1\vlhole,\xi_k\vlhole$.
...
\Psi=\vlderivation
{
 ...
 {
  \vlde{}{}{\xi_1\left\{\vlinf{=}{}{\fff}{\vls(\fff.\fff)}\right\}}
  {
   \vlde{}{}{\vdots}
   {
    \vlde{}{}{\xi_k\left\{\vlinf{=}{}{\fff}{\vls(\fff.\fff)}\right\}}
    {
...
\begin{pro}
Given two canonical experiments of a derivation, possibly under two different orderings of atoms, the atomic flows of the two experiments are equal.
\end{pro}
...
\begin{defi}
Given a set of $2n$ atoms, $\mathcal A$, which is closed under duality, consider a set, $\Choices$, of all the possible choices of $\mathcal A$, then the \emph{canonical proof of }$\bigvee_{\{a_1,\dots,a_n\}\in\Choices}\vls(a_1.\cdots.a_n)$ is defined by induction on $n$:
...
\begin{pro}
Given a proof $\Phi$ and two canonical cut-free proofs obtained from $\Phi$, possibly under two different orderings of atoms, the atomic flows of the two canonical cut-free proofs are equal.
\end{pro}
...
% TODO: ``an enumeration'': no need [ask Alessio about this comment, I don't get it]
