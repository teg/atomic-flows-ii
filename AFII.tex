...
Given a derivation $\Phi$ containing the atoms $a_1,\dots,a_n$ such that\/ $\Norm_{k-1}(\Phi,a_1,\dots,a_{k-1})$ and\/ $\Norm_k(\Phi,a_1,\dots,a_k)$ exist for some $0\leq k\leq n$, if\/ $\Norm_{k-1}(\Phi,a_1,\dots,a_{k-1})$ is weakly streamlined and none of the atoms $a_k,\dots,a_n$ occur in interaction nor cointeraction instances in\/ $\Norm_{k-1}(\Phi,a_1,\dots,a_{k-1})$ then
\begin{itemize}
