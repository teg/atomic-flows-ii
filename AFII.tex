...
The \emph{path breaker}, $\Break$, is an operator whose arguments are a derivation $\vlder{\Phi}{}{\beta}{\alpha}$ and the atom occurrences $a^\lambda$ and $a^\mu$, such that $\alpha=\vls([a^\lambda.\bar a].\gamma)$, $\beta=\vls[\delta.(a^\mu.\bar a)]$ and the results are a derivation and two atom occurrences $a^\epsilon$ and $a^\iota$ such that $\Break(\Phi,a^\lambda,a^\mu)=(\Psi,a^\epsilon,a^\iota)$, where:
\newbox\DeltaTopK
\setbox\DeltaTopK=
\hbox{$
\vlderivation
...
\vlderivation
{
 \vlin{=}{}{\vls[\vlinf{2\cdot\cod}{}{\delta}{\vls[\delta.\delta.\delta]}\;.\;(a^\iota.\bar a)]}
 {
  ...
  {
   \vlin{\swi}{}{\vls[(\gamma.a^{\epsilon'}).[\delta.\delta]]}
   {
    \vlin{=}{}{\vls(\gamma.[a^{\epsilon'}.[\delta.\delta]])}
    {
...
If the atomic flow of $\vlder{\Phi}{}{\vls[\beta.(a^{\mu}.\bar a^{\mu'})]}{\vls([a^{\lambda}.\bar a^{\lambda'}].\alpha)}$ is of shape
\[
\atomicflow
{
(-8, 6)*{\afvjm{4}};
...
\]
for some flows $\phi$ and $\psi$ and if $\Break(\Phi,a^\lambda,a^\mu)=\left(\vlder{\Psi}{}{\beta}{\alpha},a^\epsilon,a^\iota\right)$, where $\alpha=\vls([a^\epsilon.\bar a^{\epsilon'}].\gamma)$ and $\beta=\vls[\delta.(a^\iota.\bar a^{\iota'})]$, then
\begin{itemize}
...
\Tom{Changed `restriction' into `assumption'. I don't know if this improves anything... I thought it would be clear.}

\begin{remark}
A stronger version of Proposition~\ref{PropPathBreak}, without any assumptions about the atomic flow of $\Phi$, also holds. However, assuming $\phi$ and $\psi$ are disconnected makes the atomic flow of $\Psi$ easier to visualise and the stronger version of the proposition is not needed for the main result in this paper.
\end{remark}
