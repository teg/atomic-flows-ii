...
\begin{defi}
Given a derivation $\Phi$ from $\alpha$ to $\beta$, where $a_1,\dots,a_n$ are all the atoms that appear in both interaction and cointeraction instances, a \emph{core of\ $\Phi$} is defined as a derivation 
\[
...
\begin{proof}
Let $a_1,\dots,a_n$ be all the atoms which take part in both interaction and cointeraction instances in $\Phi$. Obtain $\Phi'$ from $\Phi$ by using Lemma~\ref{LemSuperSwitch} to perform the following transformation for each instance of $\aid$ and $\aiu$ applied to one of $a_1,\dots,a_n$ in $\Phi$:
\[
\vlderivation
{
 \vlde{\Psi'}{}{\beta}
...
\begin{defi}
A core of a derivation $\Phi$ constructed as described in the proof of Lemma~\ref{LemConstrCore} is called \emph{the core of $\Phi$}.
\end{defi}

\begin{defi}
The \emph{$n$-normaliser}, $\Psi$, is a derivation scheme over the meta variables $\Phi$ (derivation), $\alpha,\beta$ (formulae) and $a_1,\dots,a_n$ (atoms).

We define $\vlder{\Psi_k}{}{\vls[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i})]}{\vls(\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha)}$ inductively. Let $\Psi_1=\vlder{\Phi}{}{\vls[\beta.\bigvee_{i=1}^n\vlsbr(a_i.{\bar a_i})]}{\vls(\bigwedge_{i=1}^n\vlsbr[a_i.{\bar a_i}].\alpha)}$ and for $1<k\leq n+1$ define the three helper derivations $\Delta^\top_k,\Delta_k,\Delta^\bot_k$:

\[
\Delta^\top_k=
   \vlderivation
   {
    \vlde{\Psi_{k-1}}{}{\vls[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i}).(\vlinf{\awu}{}{\ttt}{a_{k-1}}.\bar a_{k-1})]}
    {
     \vlhy{\vls(\vlinf{\aid}{}{\vls[a_{k-1}.\bar a_{k-1}]}{\ttt}.\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha)}
    }
   }\quad,\qquad
\Delta_k=
   \vlderivation
   {
    \vlde{\Psi_{k-1}}{}{\vls[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i}).(a_{k-1}.\vlinf{\awu}{}{\ttt}{\bar a_{k-1}})]}
    {
     \vlhy{\vls([\vlinf{\awd}{}{a_{k-1}}{\fff}.\bar a_{k-1}].\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha)}
    }
   }\quad,
\]
\[
\Delta^\bot_k=
   \vlderivation
   {
    \vlde{\Psi_{k-1}}{}{\vls[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i}).\vlinf{\aiu}{}{\fff}{\vls(a_{k-1}.\bar a_{k-1})}]}
    {
     \vlhy{\vls([a_{k-1}.\vlinf{\awd}{}{\bar a_{k-1}}{\fff}].\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha)}
    }
   }\quad.
\]

Then define $\Psi_k$ to be


\newbox\DeltaTopK
\setbox\DeltaTopK=\hbox{$\vlder{\Delta^\top_k}{}{\vls[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i}).\bar a_{k-1}]}{\vls(\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha)}$}
\newbox\DeltaK
\setbox\DeltaK=\hbox{$\vlder{\Delta_k}{}{\vls[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i}).a_{k-1}]}{\vls(\bar a_{k-1}.\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha)}$}
\newbox\DeltaBotK
\setbox\DeltaBotK=\hbox{$\vlder{\Delta^\bot_k}{}{\vls[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i})]}{\vls(a_{k-1}.\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha)}$}
\[
\vlderivation
{
 \vlin{\cod^\star}{}{\vls[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i})]}
 {
  \vlin{\swi}{}{\vls[[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i})].[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i})].\box\DeltaBotK]}
  {
   \vlin{\swi}{}{\vls([[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i})].\box\DeltaK].(\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha))}
   {
    \vlin{\cou^\star}{}{\vls(\box\DeltaTopK.(\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha).(\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha))}
    {
