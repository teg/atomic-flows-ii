...
\quad,
\]
with the same atomic flow as $\Phi$, which exists by Lemma~\ref{LemDecompInt}, where $a_1,\dots,a_n$ are distinct and pairwise non-dual and $b_1,\dots,b_k$ has no atoms in common with $c_1,\dots,c_k$. A core of $\Phi$, denoted $\Core(\Phi)$, is defined to be any derivation of the form
\[
