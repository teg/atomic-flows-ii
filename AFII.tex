...
\begin{rem}
The formalism we are using in this paper suffers from a great deal of bureaucracy. We will simplify our lives by assuming two derivations to be equivalent if they have the same atomic flow up to associativity of contraction nodes. In particular this allows us to use algorithms which strictly speaking are ambiguous, but where the output will always be unique up to this criteria.
\end{rem}

% TODO: Check how this plays with the use of `canonical proof' where no algorithm is specified	

\begin{defi}
Given an algorithm which depends on an enumeration of atoms. The output of the algorithm is said to be \emph{canonical} if it is the same for all permutations of the enumariton of atoms.
\end{defi}
...
%  There are three cases to consider, where $\beta$ is not a unit:
% \begin{itemize}
...
to obtain
\[
\vlder{\Phi'}{}{\vls[\beta.(a_n.\bar a_n).\cdots.(a_n.\bar a_n).\cdots.(a_1.\bar a_1).\cdots.(a_1.\bar a_1)]}{\vls([a_1.\bar a_1].\cdots.[a_1.\bar a_1].\cdots.[a_n.\bar a_n].\cdots.[a_n.\bar a_n].\alpha)}
\]
...
\end{itemize}
an \emph{experiment on $\Phi$ with respect to $a_1,\dots,a_n$} is a derivation from $\vls(\bar a_1.\cdots.\bar a_n)$ to $\beta$ which does not contain any cointeraction instances and which has the same size as $\Phi$ up to a polynomial.
\end{defi}

\begin{defi}
Given a derivation $\Phi$ containing the atom occurrence $a$. Consider the atomic flow of $\Phi$, $A$, and the edge, $\epsilon$, in $A$ mapped to by $a$. The \emph{cone of $\epsilon$} is the edges in the paths starting with $\epsilon$ and the \emph{cone of $a$} is the set of atoms mapping to edges in the cone of $\epsilon$.
\end{defi}

\Tom{
This problem is now hidden in the definition of derivations/atomic flows:
...
\quad\rightarrow\quad
\vlinf{\awd}{}{\vls(\fff.a)}{\fff}\quad\mbox{and}
\\
\noalign{\bigskip}
...
\begin{proof}
Let $\{a_1,\dots,a_n,\bar a_1,\dots,\bar a_n\}$ be the set of atoms in $\Phi$ which occurr in both interaction and cointeraction instances and let $A$ be an enumeration of all the possible chocies of $n$ distinct, non-dual atoms from this set. Now build the canonical derivation:
\[
\vlderivation
{
 \vlde{}{\{\acd,\med\}}{\alpha}
