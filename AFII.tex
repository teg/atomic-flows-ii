...
Let $\Phi$ be a derivation from $\alpha$ to $\beta$ with atomic $A$, we then say that $\Psi$ from $\vls(\alpha^\fff.\gamma)$ to $\beta^\fff$ with atomic flow $B$ is an $\fff$-experiment on $A$ under polarity assignment $\pi$ if
\begin{enumerate}
 \item $A$ and $B$ are given by
 \item $\pi$ is such that all the edges connecting $B$ and (co)interaction nodes are negative;
 \item $\alpha^\pi$ is obtained from $\alpha$ and $\beta^\pi$ is obtained from $\beta$ by substituting atoms mapping to positive edges in $B$ by $\ttt$ and atoms mapping to negative edges in $B$ by $\fff$; and
 \item $\gamma$ is a conjunction of all the atoms taking part in the evidenced interactions in $A$.
\end{enumerate}
\end{defi}
