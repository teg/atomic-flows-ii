...
\begin{definition}
For every $n\ge0$ the \emph{normaliser of degree n}, $\Norm_n$, is an operator whose arguments are a derivation $\vlder{\Phi}{}{\beta}{\alpha}$, with core
\[
\Core(\Phi)=
\vlder{}{}{\vls[\beta.(a_1^{\iota_1}.\bar a_1^{\iota_1'}).\dots.(a_n^{\iota_n}.\bar a_n^{\iota_n'})]}{\vls([a_1^{\epsilon_1}.\bar a_1^{\epsilon_1'}].\dots.[a_n^{\epsilon_n}.\bar a_n^{\epsilon_n'}].\alpha)}\quad,
\]
a choice between $a_i^{\epsilon_i}$ and $\bar a_i^{\epsilon_i'}$, for each $0\le i\le n$, and a total order of the choices. Without loss of generality, let the order be $a_1^{\epsilon_1}$, $\dots$, $a_n^{\epsilon_n}$, then let
\[
\Psi=\Break(\Break(\dots(\Break(\Core(\Phi),a^{\epsilon_n}_n,a^{\iota_n}_n))\dots),a^{\epsilon_1}_1,a^{\iota_1}_1)\quad,
\]
and
\[
\Norm_n(\Phi,a_1^{\epsilon_1},\dots,a_n^{\epsilon_n})=
