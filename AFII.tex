...
\begin{rem}
While the definition of $\SKS$ derivations is unambiguous, we will sometimes use ambiguous representations. This is not a problem, as we can always get a more detailed representation should we need to, for instance when extracting atomic flows from derivations. The ambiguity arises because we omit or collapse equations and because we do not distinguish the different equation rules. In particular we need to know if
\[
\vlinf{=}{}{\vls((a.a).a)}{\vls(a.(a.a))}\quad,
\]
is an application of associativity or commutativity to be able to map atom occurrences in the premiss to atom occurrences in the conclusion.
\end{rem}
...
\]
Notice that the knowledge of the order in which derivations should be composed is implicit in Definition~\ref{DefDerivation}. In this example we used a left-to-right ordering, but we could just as well have used a right-to-left ordering:
\[
\vlderivation
{
 \vlin{\acd}{}{a}
...
\]
In Formalism A, a new deep-inference formalism, currently under development, the above shorthand is a representative of the equivalence class containing both the left-to-right and right-to-left versions, in addition to the intermediate:
\[
\vlderivation
{
 \vlin{\aiu}{}{a}
...
\]
In this paper, which of these three derivations we use is inessential, as the atomic flows of all the derivations in a Formalism A equivalence class are the same. For simplicity we let the shorthand above refer to one unique derivation, but should we wish to employ Formalism A, everything in this paper still holds.
\end{exa}
...
\begin{enumerate}
\item if $\Phi$ is in $\SKS\setminus\{\aid,\aiu\}$, substitute each instance of $a$ in $\Phi$ with $\fff$ and each structural inference rule where $a$ occurred with $=$,
\item otherwise, use Lemma~\ref{LemDecompInt} to obtain
\[
\vlder{\Phi'\{a\ot\fff\}}
...
\]
such that $\Phi'$ has the same atomic flow as $\Phi$ and $a=a_1,\dots,a_k,b_1,\dots,b_l$.
\end{enumerate}
...
\quad,
\]
with the same atomic flow as $\Phi$, which exists by Lemma~\ref{LemDecompInt}, where $a_1,\dots,a_n$ are distinct and pairwise non-dual atoms and $b_1,\dots,b_k$ has no atoms in common with $c_1,\dots,c_k$. A core of $\Phi$, denoted $\Core(\Phi)$, is defined to be any derivation of the form
\[
...
\]
where $A$ does not contain a path from an interaction to a cointeraction vertex, the atomic flow of a core of\/ $\Phi$ is
\[
\atomicflow
{
(-21,10)*{\afvjm{12}};
...
\begin{itemize}
\item If $\epsilon$ is an edge connecting two of $A_1,A_2,A_3,A_4,B_1,B_2,B_3$ or $B_4$ then $a_n$ or $\bar a_n$ maps to $\epsilon$, so since $\Norm(\Core(\Phi),a_1,\dots,a_{n-1})$ is weakly streamlined and contains no interaction or cointeraction rule where $a$ or $\bar a$ occurs, there is no edge $\epsilon'$ in a path from an interaction vertex to a cointeraction vertex, such that one of $a_1,\dots,a_{n-1}$ maps to $\epsilon'$.
...
\begin{rem}
There are $2^n$ different ways of choosing $a_1,\dots,a_n$ given a derivation $\Phi$.
\end{rem}
