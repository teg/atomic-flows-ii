...
\renewcommand{\gcd }{{\gc{\downarrow}}}
\newcommand  {\gcu }{{\gc{\uparrow}}}
\newcommand  {\iden  }{\mathsf i}
\newcommand  {\idd }{{\iden{\downarrow}}}
...
\vlinf\gwu{}\ttt\alpha
\quad,\qquad
\vlinf\gcd{}\alpha{\vls[\alpha.\alpha]}
\qquad\text{and}\qquad
\vlinf\gcu{}{\vls(\alpha.\alpha)}\alpha
\quad.
\]
Clearly, a derivation in $\SKS$ is also a derivation in $\SKSg$. It can easily be proved that $\SKS$ and all its fragments containing the logical and $=$ rules polynomially simulate, respectively, $\SKSg$ and its corresponding fragments \cite{BrusGugl:07:On-the-P:fk}. For example, $\{\swi,\med,=,\acd\}$ polynomially simulates $\{\swi,=,\gcd\}$, and $\aSKS=\{\swi,\med,=,\aid,\awd,\acd,\acu\}$ polynomially simulates $\{\swi,=,\idd,\gwd,\gcd,\gcu\}$ (where $\idd$ is the nonatomic identity). This allows us to transfer properties from $\SKS$ to $\SKSg$; in particular, the main result in this paper, \emph{i.e.}, that $\SKS$ proofs can be transformed into analytic ones in quasipolynomial time, holds also for $\SKSg$ proofs.
...
As we mention at the beginning of this section, atomic flows help in selectively substituting for atom occurrences. In fact, given a derivation and its associated flow, we can use edges and boxes to individuate atom occurrences inside the derivation, and then possibly substitute for them. For example, let us suppose that we are given the following associated derivation and flow:
%TODO: fix this...
\[
\Phi=
{\vlsbr[\vlderivation                                              {
...
( 2,10)*{\afvjdm4{}{}};
(10,10)*{\afvjdm4{}{}};
( 3, 7)*{\aflabelright\phi};
(11, 7)*{\aflabelright\psi};
( 2, 5)*{\affr66};
(10, 5)*{\affr66};
( 2, 0)*{\afvjum4{}{}};
(10, 0)*{\afvjum4{}{}}}
\quad,
\]
such that $\phi$ is a connected component whose edges are each associated with atom $a$ or $\bar a$; then, for any formula $\beta$, there exists a derivation
\[
...
(22,10)*{\afvjdm4{}{}};
( 3, 7)*{\aflabelright\phi};
(15, 7)*{\aflabelright\phi};
(23, 7)*{\aflabelright\psi};
( 2, 5)*{\affr66};
( 8, 5)*{\cdots};
(14, 5)*{\affr66};
(22, 5)*{\affr66};
...
%-------------------------------------------------------------------------------
\begin{remark}\label{NotDerSubst}
When the hypotheses of Proposition~\ref{PropSubst} are valid, we can describe $\Psi$ as $\Phi\{a^\phi/\beta,\bar a^\phi/\bar\beta\}$; one of $a^\phi/\beta$ or $\bar a^\phi/\bar\beta$ might be missing, when no identity or cut vertices are present in $\phi$.
\end{remark}
