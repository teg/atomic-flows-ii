...
\end{proposition}
\begin{figure}
%order the colors (top to bottom: red, green, black)
\[
\atomicflow
{
(0,-20)="D";
...
All the edges that might be in paths from $\one$ or $\two$ are colored in red and all the edges that might be in paths to $\three$ or $\four$ are colored in green. Since the red and the green paths do not meet there are no paths from $\one$ to $\three$ and no paths from $\two$ to $\four$.
\end{proof}

\begin{remark}
A stronger version of Proposition~\ref{PropFlowNorm}, without the restriction on the atomic flow of $\Phi$, also holds. However, the restriction makes the atomic flow easier to visualise and the stronger version of the proposition is not needed for the main result in this paper.
\end{remark}
%\begin{remark}
