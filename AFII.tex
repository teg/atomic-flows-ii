...
For every $n\ge 0$ the \emph{normaliser of degree $n$}, $\Norm_n$, is an operator whose arguments are the atoms $a_1,\dots,a_n$ and any derivation $\Phi$ of the form
\[
\vlder{}{}{\vls[\beta.(a_n.{\bar a_n}).\cdots.(a_1.{\bar a_1})]}{\vls([a_1.{\bar a_1}].\cdots.[a_n.{\bar a_n}].\alpha)}\quad,
\]
...
\vlderivation
{
 \vlde{\Norm_{n-1}(a_1,\dots,a_{n-1},\Phi)}{}{\vlsbr[\beta.(\vlinf{}{}{\ttt}{a_n}.\bar a_n)]}
 {
...
$\vlderivation
{
 \vlde{\Norm_{n-1}(a_1,\dots,a_{n-1},\Phi)}{}{\vlsbr[\beta.\vlinf{}{}{\fff}{\vls(a_n.\bar a_n)}]}
 {
...
\vlderivation
{
 \vlde{\Norm_{n-1}(a_1,\dots,a_{n-1},\Phi)}{}{\vlsbr[\beta.(a_n.\vlinf{}{}{\ttt}{\bar a_n})]}
 {
...
%TODO: structural rules' labels everywhere... remove them? even repeated generic contraction?
...
Let the atomic flow of $\vlder{\Norm_n(a_1,\dots,a_n,\Phi)}{}{\vls[\beta.(a^{\three}.\bar a^{\four})]}{\vls([a^{\one}.\bar a^{\two}].\alpha)}$ be
\[
\atomicflow
{
(-8, 6)*{\afvjm{4}};
...
\]
such that $a$ is mapped to all the edges in $\phi$ and $\bar a$ is mapped to all the edges in $\phi'$; then the atomic flow of $\vlder{\Norm_{n+1}(a_1,\dots,a_n,a,\Phi)}{}{\beta}{\alpha}$ is
\[
\atomicflow
{
(0,-18)="D";
...
\] 
%TODO: explain better
where all the edges that might be in paths from the shown identity vertex are colored in red and all the edges that might be in paths to the shown cut vertex are colored in green.
\end{remark}
%\begin{remark}
...
\begin{theorem}
Given a derivation $\Phi$ and all the non-weakly-streamlined atoms $a_1,\bar a_1,\dots,a_n,\bar a_n$ in $\Phi$, the derivation $\Norm_n(a_1,\dots,a_n,\Core(\Phi))$ is weakly streamlined and has the same premiss and conclusion as $\Phi$.
\end{theorem}
