...
\Tom{Added definition (long time ago).}
...
\]
For every $1\le i\le n$, the \emph{new upper edge of $\phi'_i$ (resp., $\phi''_i$)} is $\lambda'_i$ (resp., $\lambda''_i$), the \emph{new lower edge of $\phi'_i$ (resp., $\phi''_i$)} is $\mu'_i$ (resp., $\mu''_i$) and the \emph{new upper (resp., lower) edges of $\phi_i$} are $\lambda'_i$ and $\lambda''_i$ (resp., $\mu'_i$ and $\mu''_i$).
...
\]
for some flows $\phi$ and $\psi$, then there are no paths from the upper edge $\epsilon$ to the lower edge $\iota$ in the atomic flow of\/ $\Break(\Phi,a^\one,a^\three)$ if
\begin{enumerate}
	\item $\epsilon=\one$ and $\iota=\three$;
	\item $\epsilon=\two$ and $\iota=\four$; or
	\item there is no path from $\epsilon$ to $\iota$ in the atomic flow of $\Phi$.
\end{enumerate}
\end{proposition}
...
All the edges that might be in paths from $\one$ or $\two$ are coloured in red and all the edges that might be in paths to $\three$ or $\four$ are coloured in green. Since the red and the green paths do not overlap, there are no paths from $\one$ to $\three$ and no paths from $\two$ to $\four$. Furthermore, by referring to Figure~\ref{FigFlowBreak} observe that if there is a path from the upper edge $\epsilon$ to the lower edge $\iota$ the path must pass through one of the three copies of either $\phi$ or $\psi$, hence there must be a path from $\epsilon$ to $\iota$ in the atomic flow of $\Phi$.
\end{proof}
...
\Tom{Changed the next definition to use ordered components. Note that compared to the version I sent you, I am now talking about ordered choice of components from the atomic flow and not the derivation (since that is not defined).}

\newcommand{\Norm}{\mathsf{Norm}}
\begin{definition}
For every $n\ge0$ the \emph{normaliser of degree n}, $\Norm_n$, is an operator whose arguments are a derivation $\vlder{\Phi}{}{\beta}{\alpha}$, with atomic flow $\phi$, and an ordered choice of components from $\phi$, $\phi_1$, $\dots$, $\phi_n$. Let
\[
\Core(\Phi)=
\vlder{}{}{\vls[\beta.(a_1^{\iota_1}.\bar a_1).\dots.(a_n^{\iota_n}.\bar a_n)]}{\vls([a_1^{\epsilon_1}.\bar a_1].\dots.[a_n^{\epsilon_n}.\bar a_n].\alpha)}
\]
such that for every $1\le i\le n$, we have that $\epsilon_i$ (resp., $\iota_i$) is the new upper (resp., lower) edge of $\phi_i$, and let
\[
...
\[
\Norm_n(\Phi,\phi_1,\dots,\phi_n)=
\vlder{\Psi}{}{\vlsbr[\beta\;.\;\vlinf{}{}{\fff}{\vls(a^{\iota_1}_1.\bar a^{\iota'_1}_1)}\;.\;\cdots\;.\;\vlinf{}{}{\fff}{\vls(a^{\iota_n}_n.\bar a^{\iota'_n}_n)}]}{\vlsbr(\vlinf{}{}{\vls[a^{\epsilon_1}_1.\bar a^{\epsilon'_1}_1]}{\ttt}\;.\;\cdots\;.\;\vlinf{}{}{\vls[a^{\epsilon_n}_n.\bar a^{\epsilon_n}_n]}{\ttt}\;.\;\alpha)}\quad.
\]
By induction on Proposition~\ref{PropPathBreak} it follows that, for $1\le i\le n$, there are no paths from $a^{\epsilon_i}_i$ to $a^{\iota_i}_i$ and no paths from $\bar a^{\epsilon'_i}_i$ to $\bar a^{\iota'_i}_i$. It then follows, by Definition~\ref{DefFlowCore}, that $\Norm_n(\Phi,\phi_1,\dots,\phi_n)$ is weakly streamlined.
\end{proof}
