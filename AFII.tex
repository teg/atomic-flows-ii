...
% TODO: the real point is associativity
...
% TODO: don't call it choice, call it something else. assignment?
...
% TODO: too much, the last sentence is enough
\begin{rem}
...
% TODO: instead of saynig ``occuring'' say ``each of which occurs''

\begin{lem}\label{LemConstrExp}
Given a derivation $\Phi$ from $\alpha$ to $\beta$, let $\mathcal A$ be all the atoms occurring in both interaction and cointeraction instances and let $a_1,\dots,a_n$ be a choice of $\mathcal A$, then there exists an experiment on $\Phi$ with respect to $a_1,\dots,a_n$.
\end{lem}
...
\begin{itemize}
  \item substitute each occurrence of $\mk c$ with $\fff$ and
...
\]
\end{itemize}
% TODO: Make it a proper Remark (and remove the above one)
...
% TODO: don't call it the fist $k$ atoms from A
% TODO: call the proof ``beautiful'' or something like that

\begin{defi}
Given a set of $2n$ atoms, $\mathcal A$, which is closed under duality, consider a set, $\Choices$, of all the possible choices of $\mathcal A$, then the \emph{canonical proof of }$\bigvee_{\{a_1,\dots,a_n\}\in\Choices}\vlsbr(a_1.\cdots.a_n)$ is defined by induction on $n$:
