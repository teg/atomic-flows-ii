...
\]
where, for $1\le i\le n$, $\phi_i$ and $\phi'_i$ contain no identity or cut vertices and the subflow consisting of $\phi_i$, $\phi'_i$ and the identity and cut vertices connecting them is a connected, non-weakly-streamlined subflow, we define a \emph{core of $\phi$} to be
\[
\atomicflow
{
...
\begin{definition}
For every $n\ge0$ the \emph{normaliser of degree n}, $\Norm_n$, is an operator whose arguments are a derivation $\vlder{\Phi}{}{\beta}{\alpha}$, such that two atom occurrences in $\Phi$ are instances of the same or dual atoms if and only if they are connected, and an order of atoms $a_1$, $\dots$, $a_n$ such that $a_1$, $\dots$, $a_n$ and their duals are the only non-weakly-streamlined atoms in $\Phi$. Let
\[
\Core(\Phi)=
\vlder{}{}{\vls[\beta.(a_1^{\iota_1}.\bar a_1).\dots.(a_n^{\iota_n}.\bar a_n)]}{\vls([a_1^{\epsilon_1}.\bar a_1].\dots.[a_n^{\epsilon_n}.\bar a_n].\alpha)}\quad,
\]
and let
\[
...
\begin{theorem}
Given a derivation $\Phi$, such that two atom occurrences in $\Phi$ are instances of the same or dual atoms if and only if they are connected, and all the non-weakly-streamlined atoms $a_1$, $\bar a_1$, $\dots$, $a_n$, $\bar a_n$ in $\Phi$, the derivation $\Norm_n(a_1,\dots,a_n,\Phi)$ is weakly streamlined and has the same premiss and conclusion as $\Phi$.
\end{theorem}
