...
\vlderivation
{
 ...
 {
  \vlhy{\vlsbr(\vlinf{}{}{\vls[a.\bar a]}{\ttt}.\alpha)}
 }
}$
}
\newbox\DeltaK
...
$\vlderivation
{
 \vlde{\Phi}{}{\vlsbr[\beta.\vlinf{}{}{\fff}{\vls(a.\bar a)}]}
 {
...
\]
then the atomic flow of $\vlder{\Break(a,\Phi)}{}{\beta}{\alpha}$ is
\[
\atomicflow
{
(0,-18)="D";
...
\] 
%TODO: explain better
where all the edges that might be in paths from the shown identity vertex are colored in red and all the edges that might be in paths to the shown cut vertex are colored in green. Note that the red and the green paths never meet, so there is no path from the shown identity vertex to the shown cut vertex and if $\phi$ and $\phi'$ contain no interaction or cut vertices, then $a$ is weakly streamlined.
\end{remark}
%\begin{remark}
...
\begin{definition}
For every $n\ge0$ the \emph{normaliser of degree n}, $\Norm_n$, is an operator whose arguments are the atoms $a_1$, $\dots$, $a_n$ and a derivation $\Phi$, such that $a_1$, $\bar a$, $\dots$, $a_n$, $\bar a_n$ are all the non-weakly-streamlined atoms in $\Phi$. $\Norm_n(a_1,\dots,a_n,\Phi)$ is defined to be $\Break(a_1,\Break(\dots(\Break(a_n,\Core(\Phi)))\dots))$.
\end{definition}
