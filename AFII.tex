...
\TODO{Complete the first definition}

Obtain the core of $\Phi$ from $\Phi'$ by, for each atom $a$;
...
\vlderivation
{
 ...
 {
  \vlin{\swi}{}{\vls[[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i})].[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i})].[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i})].\Delta'''_k]}
  {
   \vlin{\swi}{}{\vls([[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i})].[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i})].\Delta''_k].(\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha))}
   {
    \vlin{\swi}{}{\vls([[\beta.\bigvee_{i=k}^n\vlsbr(a_i.{\bar a_i})].\Delta'_k].(\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha).(\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha))}
    {
     \vlin{\cou^\star}{}{\vls(\Delta_k.(\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha).(\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha).(\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha))}
     {
      \vlhy{\vls(\bigwedge_{i=k}^n\vlsbr[a_i.{\bar a_i}].\alpha)}
     }
    }
...
\begin{defi}
Given a derivation $\Phi$ and given the atoms in $\Phi$, $a_1,\dots,a_n$ which are not streamlined, the \emph{normaliser of $\Phi$} is the derivation obtained by instantiating the $n$-normaliser with the core, premiss and conclusion $\Phi$ as well as the atoms $a_1,\dots,a_n$.
\end{defi}
