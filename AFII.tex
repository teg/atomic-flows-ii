...
\]
By comparing the flow associated with $a$ and the flow associated with $b$ in $\Norm_2(a,b,\Core(\Phi))$ we can see why the procedure is non-confluent. We could trivially change our procedure to make the subflows more simial by cocontracting (resp., contracting) the interaction (resp., cut) vertices in the first subflow and permuting the weakening (resp., coweakening) vertices down (resp., up) in the second subflow. However, the subflows marked in red will still differ.
\end{example} 
%===========================================
\section{Conclusion}
