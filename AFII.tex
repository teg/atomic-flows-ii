...
%TODO: Check that it is ok to say "an edge from x to y" (taken from Wikipedia). at the moment the path is directed, in AFI it was not. this we can fix, but we should check the rest of the paper.
...
%COMMENT: what does it mean? Proposal: write a direct explanation: 'Normalisation works like this: * we remove... *we add back...'
...
The \emph{path breaker}, $\Break$, is an operator whose arguments are the atom $a$ and any derivation $\Phi$ of the form
\[
\vlder{}{}{\vls[\beta.(a.\bar a)]}{\vls([a.\bar a].\alpha)}\quad,
\]
and whose output is
\newbox\DeltaTopK
\setbox\DeltaTopK=
...
\vlderivation
{
 \vlde{\Phi}{}{\vlsbr[\beta.(\vlinf{}{}{\ttt}{a}.\bar a)]}
 {
  \vlhy{\vls([a.\bar a].\alpha)}
 }
}$
}
\newbox\DeltaK
...
\vlderivation
{
 \vlde{\Phi}{}{\vlsbr[\beta.(a.\vlinf{}{}{\ttt}{\bar a})]}
 {
  \vlhy{\vlsbr([\vlinf{}{}{a}{\fff}.\bar a].\alpha)}
 }
}$
}
\newbox\DeltaBotK
...
$\vlderivation
{
 \vlde{\Phi}{}{\vls[\beta.(a.\bar a)]}
 {
  \vlhy{\vlsbr([a.\vlinf{}{}{\bar a}{\fff}].\alpha)}
 }
}$
}
\[
\vlderivation
{
 \vlin{=}{}{\vls[\vlinf{}{}{\beta}{\vls[\beta.\beta.\beta]}.(a.\bar a)]}
 {
  \vlin{\swi}{}{\vlsbr[\beta\;\;\;.\;\;\;\beta\;\;\;.\;\;\;\box\DeltaBotK]}
  {
   \vlin{\swi}{}{\vlsbr([\beta\;\;\;.\;\;\;\box\DeltaK].\alpha)}
   {
    \vlin{=}{}{\vlsbr(\box\DeltaTopK\;\;\;.\;\;\;\alpha\;\;\;.\;\;\;\alpha)}
    {
     \vlhy{\vls([a.\bar a].\vlinf{}{}{\vls(\alpha.\alpha.\alpha)}{\alpha})}
    }
   }
  }
 }
...
%TODO: structural rules' labels everywhere... remove them? even repeated generic contraction? (need to change the remark where we define them if we do)

%TODO: repeated use of labels? missing labels... superposition of colors. (co)weakening vertices are too close
...
\]
then the atomic flow of $\vlder{\Break(a,\Phi)}{}{\vls[\beta.(a^{\three}.\bar a^{\four})]}{\vls([a^{\one}.\bar a^{\two}].\alpha)}$ is
\[
\atomicflow
{
(0,-18)="D";
...
% join one
"A"+(2,-6.5)*{\afvjdcol5{}{\four'}{Red}};
"A"+(2,-11.5)*{\afvjucol5{}{\two''}{Red}};
% middle boxes
"A"+"D"="A";
...
"A"+( 6, 2)*{\aflabelright{\phi'}};
% join two
"A"+(-2,-6.5)*{\afvjdcol5{\three''}{}{OliveGreen}};
"A"+(-2,-11.5)*{\afvjucol5{\one'}{}{OliveGreen}};
% bottom boxes
"A"+"D"="A";
...
\] 
where all the edges that might be in paths starting with $\one$ or $\two$ are colored in red and all the edges that might be in paths ending with $\three$ or $\four$ are colored in green. Note that the red and the green paths never meet, so there is no path from $\one$ to $\three$ or from $\two$ to $\four$.
\end{remark}
%\begin{remark}
%Dually, if we work on $\bar a$ instead of $a$, the atomic flow of $\vlder{\Break_{n+1}(a_1,\dots,a_n,\bar a,\Phi)}{}{\beta}{\alpha}$ is
%\[
%\atomicflow
%{
%(0,-18)="D";
...
%\end{remark}
%TODO: if reinstating the above remark, check comments from Alessio on 21/4/09

%TODO: check if we want to rephrase this since things changed:
...
\begin{definition}
For every $n\ge0$ the \emph{normaliser of degree n}, $\Norm_n$, is an operator whose arguments are the atoms $a_1$, $\dots$, $a_n$ and a derivation $\vlder{\Phi}{}{\beta}{\alpha}$, such that $a_1$, $\bar a$, $\dots$, $a_n$, $\bar a_n$ are all the non-weakly-streamlined atoms in $\Phi$. Let $\Psi=\Break(a_1,\Break(\dots(\Break(a_n,\Core(\Phi)))\dots))$, then $\Norm_n(a_1,\dots,a_n,\Phi)$ is defined to be
\[
...
\begin{proof}
%TODO:redo
%In the atomic flow in Remark~\ref{RemFlowNorm}, observe that, for $1 \le i\le n$
%\begin{itemize}
