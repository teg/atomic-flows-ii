...
%WOW!
The newfound simplicity allowed us to find a strongly normalising streamlining procedure as well as a strongly normalising and confluent cut-elimination procedure.
...
The idea behind the normalisation procedures we are about to present is to copy the derivation we wish to normalise, cut away parts of each copy and combine all the copies to produce a derivation on normal form. We will now establish some preliminary results we will use later on.
...
It is a fortunate consequence of having atomic inference rules that we can trivially extract all the axioms to the top and all the cuts to the bottom of the derivation.

% TODO: this can not be in this section, atomic flows have yet to be defined
%Note that this transformation does not affect the atomic flow since we are only adding linear inference rules and leaving the structural ones alone.
...
It will be convenient to assume that we always have the axioms at the top and the cuts at the bottom of a derivation. This is possible by Lemma~\ref{LemDecompInt} and furthermore the atomic flow associated with the derivation is not affected:
