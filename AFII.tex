...
\begin{itemize}
 \item substitute each formula $\xi$ with $\vls[\xi'.\delta_i]$; where $\xi'$ is obtained by substituting $a$ with $\ttt$ in $\xi$, $i$ is the number of cointeractions applied to $a$ above $\xi$ in $\Phi$, $\delta_0=\fff$ and $\delta_i=\vls[\delta_{i-1}.{\bar a}]$;
 \item let $\vlinf{\rho}{}{\zeta}{\xi}$ be a linear inference rule or a structural inference rule not applied to $a$, then it is substituted with $\vlinf{\rho}{}{\vls[\zeta'.\delta_i]}{\vls[\xi'.\delta_i]}$;
 \item let $\vlinf{\rho}{}{\zeta}{\xi}$ be a structural inference rule applied to $a$, then it is substituted with $\vlinf{=}{}{\vls[\zeta'.\delta_i]}{\vls[\xi'.\delta_i]}$\footnote{we might need a special case for $\vlinf{\awd}{}{\xi'\{\ttt\}}{\xi'\{\fff\}}$};
 \item the rule $\vlinf{\aid}{}{\xi\{\vls[a.{\bar a}]\}}{\xi\{\ttt\}}$ is substituted with $\vlinf{\awd}{}{\vls[\xi'\{\vls[\ttt.{\bar a}]\}.\delta_i]}{\vls[\xi'\{\ttt\}.\delta_i]}$; and
 \item the rule $\vlinf{\aiu}{}{\xi\{\fff\}}{\xi\{\vls(a.{\bar a})\}}$ is substituted with $\vlinf{\sus}{}{\vls[\xi'\{\fff\}.\delta_{i+1}]}{\vls[\xi'\{\vls(\ttt.{\bar a})\}.\delta_i]}$.
\end{itemize}
\end{defi}
...
\begin{itemize}
 \item substitute each formula $\xi$ with $\vls(\xi'.\delta_i)$; where $\xi'$ is obtained by substituting $a$ with $\fff$ in $\xi$, $i$ is the number of interactions applied to $a$ below $\xi$ in $\Phi$, $\delta_0=\ttt$ and $\delta_i=\vls(\delta_{i-1}.{\bar a})$;
 \item let $\vlinf{\rho}{}{\zeta}{\xi}$ be a linear inference rule or a structural inference rule not applied to $a$, then it is substituted with $\vlinf{\rho}{}{\vls[\zeta'.\delta_i]}{\vls[\xi'.\delta_i]}$;
 \item let $\vlinf{\rho}{}{\zeta}{\xi}$ be a structural inference rule applied to $a$, then it is substituted with $\vlinf{=}{}{\vls[\zeta'.\delta_i]}{\vls[\xi'.\delta_i]}$\footnote{we might need a special case for $\vlinf{\awu}{}{\xi'\{\ttt\}}{\xi'\{\fff\}}$};
 \item the rule $\vlinf{\aid}{}{\xi\{\vls[a.{\bar a}]\}}{\xi\{\ttt\}}$ is substituted with $\vlinf{\sus}{}{\vls[\xi'\{\vls[\ttt.{\bar a}]\}.\delta_i]}{\vls[\xi'\{\ttt\}.\delta_{i+1}]}$; and
 \item the rule $\vlinf{\aiu}{}{\xi\{\fff\}}{\xi\{\vls(a.{\bar a})\}}$ is substituted with $\vlinf{\awu}{}{\vls[\xi'\{\ttt\}.\delta_i]}{\vls[\xi'\{\vls(\ttt.{\bar a})\}.\delta_i]}$.
\end{itemize}
\end{defi}

\TODO{Change the rest of the paper to reflect the new definition.}
...
For any proof\/ $\vlproof{\Pi}{\SKS}{\beta}$ and any polarity assignment to its atomic  flow, $\pi$, the $\top$-experiment on $\Pi$ under $\pi$ is of the form $\vlder{}{\SKS\setminus\{\aid,\aiu\}}{\beta'}{\gamma}$. Where $\beta'$ and $\gamma$ are given by Definition~\ref{DefExperiment}.
\end{pro}
