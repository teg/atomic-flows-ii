...
\Tom{In previous definition: The ordered choice of components is given by the definition and needs not be made explicit.}

\Tom{`Make 4.3 and 4.4 into a remark, use this in def 4.5'. I haven't figured out how to do this elegantly.}
...
By Proposition~\ref{PropAssoContr} we can consider a core of an atomic flow to be unique and we therefore refer to \emph{the core}.
...
\Tom{Changed definition to make sure that the atom occurrences in the premiss and in the conclusion pair up the way they should.}
...
The core of a derivation does not have the same premiss and conclusion as the original derivation. In the next subsection we see how we can glue several cores together to obtain a weakly streamlined derivation with the correct premiss and conclusion.
...
Given a derivation, we can construct several cores with trivial differences. However, we show how a core can be extracted, and we will single out the core obtained in this way and refer to it as \emph{the core}.
...
The \emph{path breaker}, $\Break$, is an operator whose arguments are a derivation $\vlder{\Phi}{}{\beta}{\alpha}$ and the atom occurrences $a^\lambda$ and $a^\mu$, such that $\alpha=\vls([a^\lambda.\bar a].\gamma)$, $\beta=\vls[\delta.(a^\mu.\bar a)]$ and
\newbox\DeltaTopK
\setbox\DeltaTopK=
\hbox{$
\vlderivation
...
\vlderivation
{
 \vlin{=}{}{\vls[\vlinf{2\cdot\cod}{}{\delta}{\vls[\delta.\delta.\delta]}\;.\;(a^\mu.\bar a)]}
 {
    ...
    \vlin{=}{}{\vls(\gamma.[a.[\delta.\delta]])}
    {
     \vlin{=}{}{\vlsbr([\delta\;\;\;\;\;.\;\;\;\;\;\box\DeltaK]\;\;\;\;\;.\;\;\;\;\;\gamma)}
     {
      \vlin{=}{}{\vls(\vlinf{\swi}{}{\vls[(\gamma.\bar a).\delta]}{\vls(\gamma.[\bar a.\delta])}\;.\;\gamma)}
      {
       ...
       {
        \vlhy{\vls([a^\lambda.\bar a]\;.\;\vlinf{2\cdot\cou}{}{\vls(\gamma.\gamma.\gamma)}{\gamma})}
       }
      }
     }
    }
...
\Tom{Changed the next proposition.}
...
\]
for some flows $\phi$ and $\psi$, then there are no paths from the upper edge $\lambda$ to the lower edge $\mu$ in the atomic flow of\/ $\Break(\Phi,a^\one,a^\three)$ if
\begin{enumerate}
	\item $\lambda=\one$ and $\mu=\three$;
	\item $\lambda=\two$ and $\mu=\four$; or
	\item there is no path from $\lambda$ to $\mu$ in the atomic flow of $\Phi$.
\end{enumerate}
\end{proposition}
...
All the edges that might be in paths from $\one$ or $\two$ are coloured in red and all the edges that might be in paths to $\three$ or $\four$ are coloured in green. Since the red and the green paths do not overlap, there are no paths from $\one$ to $\three$ and no paths from $\two$ to $\four$. Furthermore, by referring to Figure~\ref{FigFlowBreak} observe that if there is a path from the upper edge $\lambda$ to the lower edge $\mu$ the path must pass through one of the three copies of either $\phi$ or $\psi$, hence there must be a path from $\lambda$ to $\mu$ in the atomic flow of $\Phi$.
\end{proof}

\begin{remark}
A stronger version of Proposition~\ref{PropPathBreak}, without any restrictions on the atomic flow of $\Phi$, also holds. However, assuming $\phi$ and $\psi$ are disjoint makes the atomic flow of $\Break(\Phi,a^\one,a^\three)$ easier to visualise and the stronger version of the proposition is not needed for the main result in this paper.
\end{remark}

\subsection{The Normaliser}\label{SectNorm}

\Tom{Talking about ordered choice of components from the atomic flow and not the derivation.}
...
\Tom{Changed Theorem due to changes in def.}
...
\Tom{Changed Example due to changes in def.}
