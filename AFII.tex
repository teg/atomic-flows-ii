...
\end{itemize}
an \emph{experiment on $\Phi$ with respect to $a_1,\dots,a_n$} is a derivation $\vlder{}{\SKS\setminus\{\aid,\aiu\}}{\beta}{\vls(a_1.\vldots.a_n)}\quad.$
\end{defi}

\begin{lem}\label{LemExperimentExistence}
Given a proof, $\vlproof{\Phi}{}{\beta}$, and atoms $a_1,\dots,a_n$, such that in every interaction rule instance exactly one of $a_1,\dots,a_n$ appears and every $a_1,\dots,a_n$ occurs in an interaction rule instance, then there exists an experiment on $\Phi$ with respect to $a_1,\dots,a_n$.
\end{lem}

\begin{proof}
Let $b_1,\dots,b_m$ be the atoms occuring in $\Phi$ which do not occur in interaction rule instances, then an experiment on $\Phi$ with respect to $a_1,\dots,a_n$ can be constructed:
\[
...
\begin{rem}
We could change Lemma~\ref{LemExperimentExistence} to make an experiment with respect to only the atoms occuring in both interaction and cointeraction instances, but we chose not to for efficiency of presentation.
\end{rem}
...
\[
\vlinf{\aiu}{}{\fff}{\vls(\vlinf{\awd}{}{b_i}{\fff}.\vlinf{\awd}{}{\bar b_i}{\fff})}\quad.
\]
\end{rem}
...
Given distinct and pairwise non-dual atoms, $b_1,\dots,b_n$, let $\Assignments_k=\{\{a_1,\dots,a_k\}|a_i\in\{b_i,\bar b_i\}\}$ for $0\leq k\leq n$, then a \emph{symmetric proof of }$\bigvee_{\{a_1,\dots,a_k\}\in\Assignments_k}\vlsbr(a_1.\cdots.a_n)$, denoted $\Sym(b_1,\dots,b_k)$, is defined by induction on $k$:

The base case, $k=0$, is trivial and the inductive case is:
\[
\newbox\DerCap
\setbox\DerCap=
\hbox{$
\vlderivation
{
 \vlde{}{\{\acu,\med\}}{\bigwedge_{i=1}^{2^{k-1}}[a_k.\bar a_k]}
 {
  \vlin{\aid}{}{\vls[a_k.\bar a_k]}
  {
   \vlhy{\ttt}
  }
 }
...
\vlderivation
{
 \vlde{}{\{\acu,\med\}}{\bigwedge_{i=1}^{2^{k-1}}\vls[a_k.\bar a_k]}
 {
  \vlin{\aid}{}{\vls[a_k.\bar a_k]}
  {
   \vlhy{\ttt}
...
\Sym(b_1,\dots,b_k)\quad=\quad
\vlderivation
{
 \vlin{=}{}{\bigvee_{\{a_1,\dots,a_k\}\in \Assignments_k}\vlsbr(a_1.\cdots.a_k)}
 {
  \vlde{}{\{\swi\}}{\bigvee_{\{a_1,\dots,a_{k-1}\}\in \Assignments_{n-1}}\vlsbr[(a_1.\cdots.a_{k-1}.a_n).(a_1.\cdots.a_{k-1}.\bar a_k)]}
  {
  \vlpr{\Sym(b_1,\dots,b_{k-1})}{\{\aid,\acu,\swi,\med\}}{\vls(\box\DerCap.\bigvee_{\{a_1,\dots,a_{k-1}\}\in \Assignments_{k-1}}(\vlinf{\acu}{}{\vls(a_1.a_1)}{a_1}.\cdots.\vlinf{\acu}{}{\vls(a_{k-1}.a_{k-1})}{a_{k-1}}))}
  }
 }
}\quad.
\]
...
Note that the derivation $\Sym(b_1,\dots,b_k)$ is not unique, but it's atomic flow is unique modulo associativity of contractions:
