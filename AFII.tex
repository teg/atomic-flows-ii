...
\]
where the boxes represent any atomic flow not containing axioms nor cuts. We call an atomic flow \emph{$\pmi$-prenormalised under $\pi$} if it is $\ppl$-prenormalised under the dual of $\pi$. The definitions extend to derivations in the obvious way.
\end{defi}

\begin{lem}
Given a derivation $\Phi$ from $\alpha$ to $\beta$. Then there exist a $\ppl$-prenormalised derivation $\Phi^+$ and a $\pmi$-prenormalised derivation $\Phi^-$ from $\alpha$ to $\beta$ such that $|\Phi^+|,|\Phi^-|\in O(|\Phi|)$.
\end{lem}
\begin{proof}
We prove the case for $\pmi$-prenormalised, the other case trivially follows.
...
\]
where $B_1,B_2,B_3,B_4$ are isomorphic to $B$. It is straightforward to observe that this atomic flow is $\ppl$-prenormalised.

We build $\Phi^+$ by applying substitutions to $\Phi$. For every atom occurence $a$ which maps to an edge in $B$ we substitute it with the formula $\vls[(a_1.a_2).(a_3.a_4)]$. Where $a=a_1=a_2=a_3=a_4$ and the subscripts denote which of the $B_1,B_2,B_3,B_4$ the atom maps to. Rule instances $\vlinf{\aid}{}{\vls[\bar{a}.a]}{\ttt}$ and $\vlinf{\aiu}{}{\fff}{\vls(\bar{a}.a)}$ where $a$ maps to an edge in $B$ are substituted with derivations
\[
\vlderivation
{
\vlin{\acd}{}{\vls[\bar{a}.(a_1.a_2).(a_3.a_4)]}
 {
 \vlin{\awd\times2}{}{\vls[\bar{a}.\bar{a}.(a_1.a_2).(a_3.a_4)]}
  {
  \vlin{\med}{}{\vls[\bar{a}.\bar{a}.(a_1.\fff).(\fff.a_4)]}
   {
   \vlin{=}{}{\vls[\bar{a}.\bar{a}.([a_1.\fff].[\fff.a_4])]}
    {
    \vlin{\swi\times2}{}{\vls[\bar{a}.\bar{a}.(a_1.a_4)]}
     {
     \vlin{\aid\times2}{}{\vls([\bar{a}.a_1].[\bar{a}.a_4])}
      {
      \vlhy{\ttt}
      }
     }
...
\vlin{\aiu\times2}{}{\fff}
 {
 \vlin{\swi\times2}{}{\vls[(\bar{a}.a_2).(\bar{a}.a_4)]}
  {
  \vlin{=}{}{\vls(\bar{a}.\bar{a}.[a_2.a_4])}
   {
   \vlin{\awu\times2}{}{\vls(\bar{a}.\bar{a}.[(\ttt.a_2).(\ttt.a_4)])}
    {
    \vlin{\acu}{}{\vls(\bar{a}.\bar{a}.[(a_1.a_2).(a_3.a_4)])}
     {
...
\]
respectively. Finally any occurance of $\vls[(a_1.a_2).(a_3.a_4)]$ in the premiss and $\vls[(a_1.a_2).(a_3.a_4)]$ in the conclusion are substituted with the derivations
\[
\vlderivation
{
 \vlin{\awd\times2}{}{\vls[(a_1.a_2).(a_3.a_4)]}
 {
  \vlin{\med}{}{\vls[(\fff.a_2).(a_3.\fff)]}
  {
   \vlin{=}{}{\vls([\fff.a_3].[a_2.\fff])}
   {
    \vlin{\acu}{}{\vls(a_3.a_2)}
    {
     \vlhy{a}
    }
   }
...
\vlderivation
{
 \vlin{\acd}{}{a}
 {
  \vlin{=}{}{\vls[a_1.a_3]}
  {
   \vlin{\awu\times2}{}{\vls[(a_1.\ttt).(a_3.\ttt)]}
   {
...
\]
respectively. It is now straightforward to verify that the derivation so obtained has the aforementioned atomic flow.
\end{proof}
